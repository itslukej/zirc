...
class Bot(zirc.Client):
    def __init__(self):
        ...
        self.config = zirc.IRCConfig(host="irc.freenode.net", port=6667, nickname="zIRC-CI-"+str(sys.version.split(" ")[0].replace(".", "-").replace("+", "")), ident="ci", realname="zIRC IRCP Library", channels=[], caps=zirc.Caps(zirc.Sasl(username="zIRC", password=os.environ['irc_pass'])))
