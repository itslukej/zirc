...
class TestCase(object):
    def start(self, log):
        ...
        for line in log:
            try:
                event = Event(line)
                print_("Parsed line '{0}'\n".format(line), flush=True)
            except:
            ...
            if hasattr(self, "on_all"):
                print_("Attempting to run on_all...\n", flush=True)
            ...
            if hasattr(self, text_type_func_name):
                print_("Attempting to run {0}\n".format(text_type_func_name), flush=True)
            ...
            if raw_type_func_name != text_type_func_name:
                if hasattr(self, raw_type_func_name):
                    print_("Attempting to run {0}\n".format(raw_type_func_name), flush=True)
